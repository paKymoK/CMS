...
@RequestMapping("/v1/admin/primary-nav-items")
public class PrimaryNavItemController {
  private final PrimaryNavItemService primaryNavItemService;
  private final SiteService siteService;
  private final CmsAdminGuard cmsAdminGuard;

  @GetMapping("")
  public Mono<ResultMessage<List<PrimaryNavItem>>> get(
      @RequestParam String site, Authentication authentication) {
    return authorizedSite(site, authentication)
        .flatMap(resolved -> primaryNavItemService.getAll(resolved.getId()))
        .map(ResultMessage::success);
  }

  @GetMapping("/{id}")
  public Mono<ResultMessage<PrimaryNavItem>> getById(
      @PathVariable Long id, @RequestParam String site, Authentication authentication) {
    return authorizedSite(site, authentication)
        .flatMap(resolved -> primaryNavItemService.getById(id, resolved.getId()))
        .map(ResultMessage::success);
  }

  @PostMapping("")
  public Mono<ResultMessage<PrimaryNavItem>> create(
      @RequestParam String site,
      @Valid @RequestBody PrimaryNavItemCreateRequest request,
      Authentication authentication) {
    return authorizedSite(site, authentication)
        .flatMap(resolved -> primaryNavItemService.create(resolved.getId(), request))
        .map(ResultMessage::success);
  }

  @PutMapping("")
  public Mono<ResultMessage<PrimaryNavItem>> update(
      @RequestParam String site,
      @Valid @RequestBody PrimaryNavItemUpdateRequest request,
      Authentication authentication) {
    return authorizedSite(site, authentication)
        .flatMap(resolved -> primaryNavItemService.update(resolved.getId(), request))
        .map(ResultMessage::success);
  }
  ...
  public Mono<ResultMessage<Void>> delete(
      @PathVariable Long id, @RequestParam String site, Authentication authentication) {
    return authorizedSite(site, authentication)
        .flatMap(resolved -> primaryNavItemService.delete(id, resolved.getId()))
