...
### Phase 3 status: done (except the deferred audit) — 2026-09-18

- **No new `site` table in `media-service`**, mirroring the Phase 1 discovery: media-service isn't the source of truth for which site codes are real (content-service's `site` table is), so a plain `site_id character varying` column on `upload_file` and `video_job` is enough — validated against the caller's JWT `project_roles`/`roles` claims via a new `CmsAdminGuard` (same shape as content-service's, minus the Site-entity resolution step it needs for Host-header lookups that media-service has no equivalent of).
- All three upload surfaces that write into `upload_file` — single/multiple image upload (`UploadFileController`), and the chunked large-file upload path (`ChunkedUploadController`/`ChunkedUploadServiceImpl`) — now require `?site=<code>` and a `CmsAdminGuard.requireSiteAccess` check before writing, and tag the row with that site.
- **Found and fixed a real cross-site leak while doing this, in scope for this phase**: the chunked-upload API's `GET /v1/upload/chunked/files` (and its delete-by-name / delete-all counterparts) listed and deleted **every** finished upload on disk, unscoped by site — any authenticated user, regardless of which site's `ADMIN` role they held, could browse or delete another site's files through this path even though the single/multiple upload endpoint was being scoped. Fixed by filtering the on-disk listing against `upload_file.site_id` and rejecting deletes whose row doesn't belong to the caller's site (silently no-ops rather than erroring, so a guessed/foreign id doesn't confirm its existence).
- Video pipeline (`VideoController`, `JobController`, `TranscodeJobService`, `VideoJob`/`video_job`) got the same treatment: `site_id` added to `video_job`, upload/list/delete/job-status all gated by `CmsAdminGuard` + site ownership checks. The transcode/HLS packaging layer itself (`FfmpegTranscodeService`, `HlsPackagerService`, `VideoStorageService`) is untouched — it's keyed purely by `videoId` and has no site-awareness need, same as before.
- **New media library listing endpoints** for the Phase 4 admin picker: `GET /v1/upload?site=<code>` (images + finished chunked files, since they share the `upload_file` table) and `GET /v1/videos?site=<code>` (video jobs).
- Public serving (`GET /images/**`, `GET /files/**`, `GET /v1/videos/**` HLS playback) is deliberately **not** site-gated, same reasoning as before the fork: these are content-addressed by an unguessable UUID and are public once a site's published page embeds the URL — the site boundary that matters is on the admin-side upload/list/delete operations, not on serving bytes for a URL someone already has.
- **Correction to Phase 0/2's own record**: those status notes described `content-service` as having no tests yet without commenting on `media-service`, but `media-service` in fact already carries two pre-fork test classes (`UploadSessionRegistryTest`, `ChunkedUploadServiceImplTest`, 26 tests total) that exercise the chunked-upload path in detail. These were updated to the new site-scoped signatures — not deleted — per this repo's own "a failing pre-existing test is a regression" rule, plus two new tests added for the leak found above (`deleteFile_doesNotRemoveAnotherSitesFile`, `deleteAllFiles_onlyRemovesTheGivenSitesFiles`).
- Verified: `:auth-service:compileJava`, `:media-service:compileJava`, `:chat-service:compileJava`, `:content-service:compileJava` all succeed together. `:media-service:test` passes (26/26, including the 2 new ones). `:auth-service:test`/`:content-service:test` remain vacuous (no test sources), unchanged from Phase 2.
- **Not done, deferred**: the full security audit of the forked `media-service` code (same deferral as Phase 1's auth-service audit — still waiting on the user's own separate security pass). Actually exercising `docker-compose up` end-to-end, including a real FFmpeg transcode, has still never been done in any phase — only compile/unit-test verification. The acceptance criterion's literal "upload an image and a video for vn, confirm neither appears browsing jp's media library" therefore hasn't been run against a live stack yet; it's covered at the unit-test level (repository/service scoping) but not end-to-end.
...
### Phase 4 status: done — 2026-09-18

- `admin-app` scaffolded as a **standalone** Vite + React 19 + TypeScript app — Ant Design 6, Tailwind 4, TanStack Query, react-router-dom 7, matching the plan's stack choice. Not a pnpm workspace member of Workflow's frontend (`@takypok/shared` isn't reused as a package dependency, since this is a separate repo) — the auth/API-client patterns it needs were read from `takypok-frontend`/`packages/shared` and re-implemented locally in `admin-app/src/{auth,lib}`, adapted for `cms-admin`'s own public-client (PKCE, no secret) OAuth2 flow.
- **Simplified the PKCE flow** vs. the source: Workflow's `AuthProvider`/`Callback` support both a popup-window flow (`postMessage` between windows) and a full-page redirect, because Workflow embeds login in another host page. `admin-app` is a standalone SPA with no such embedding need, so it only implements the plain redirect flow — less code, same security properties (PKCE + state check), and `refreshAccessToken` was corrected to NOT send a `Basic` auth header (Workflow's confidential `workflow` client needs one; `cms-admin` is PKCE-only with no secret to send).
- **Site switcher**: `useSiteAccess` derives `{ isGlobalAdmin, accessibleSites }` from the JWT's `roles`/`project_roles` claims (same claims Phase 1 wired up), and `SiteProvider`/`useSite` persist the chosen site in `localStorage`, defaulting to the user's first accessible site. An account with no site access at all sees an explicit empty-state instead of a broken UI.
- **One generic, config-driven CRUD page** (`ContentPage.tsx` + `config/resources.ts`) drives all 11 content-service resources, rather than 11 hand-written near-duplicate page files — a deliberate, noted deviation from the backend's "no generic abstraction, start concrete" convention: on the frontend these 11 screens really are identical in shape (same list/create/update/delete contract, confirmed by reading every controller in Phase 2), so a shared component removes real duplication rather than papering over accidental similarity. Field types (`text`/`textarea`/`number`/`boolean`/`select`/`image`/`datetime`/`string-list`/`json`) cover every field across all 11 resources' Create/Update DTOs (read directly from `content-service`'s actual request records, not guessed).
- `MediaPickerModal` (used from an "image" field) and a full `MediaLibraryPage` (images + videos, upload + delete) both call `media-service` directly (`GET/POST /v1/upload`, `GET/POST /v1/videos`, `DELETE /v1/videos/{id}`) — video preview uses `hls.js` against the HLS `master.m3u8` URL, matching how the public site would play it back.
- No admin-side delete for single/multiple-uploaded images was built, because `media-service` doesn't expose one (only the chunked-upload path has a delete endpoint, per Phase 3) — not adding UI for an endpoint that doesn't exist, rather than silently failing.
- Verified: `npm install` (297 packages), `npx tsc -b` (clean), `npx vite build` (succeeds, one single ~650KB gzipped bundle — manual chunking was deliberately dropped per the plan's own note, acceptable for an internal admin tool), `npx eslint .` (clean, after fixing two real `react-hooks`-plugin findings: a ref mutated during render in `AuthProvider`, and a `setState`-during-effect in the site resolver — both fixed properly, not suppressed).
- **Not done**: no live run against a real backend (no `docker-compose up` in this environment, consistent with every prior phase), and no automated frontend tests were added (none existed to preserve, matching content-service's own "don't pad test suites" precedent from Phase 2). A basic user-management screen was not built — the plan listed it as "if in scope," and Phase 1 noted per-site editor account creation depends on this UI existing, which is a bootstrapping chicken-and-egg the plan already flagged as future work, not resolved here.
...
### Phase 5 status: done — 2026-09-18

- **Additive, not a swap**: the pre-existing employee-facing CRM assistant (`AssistantService`, `AiConfig.chatClient`, `IngestionService`, `CodeReviewService`, the whole `/assist/**` session/employee-directory/presence/FCM apparatus) is **untouched** — still Ollama/qwen3.5, still Vietnamese-only, still backed by its original single `crm_vi` Qdrant collection. None of that is part of the CMS's public marketing chatbot, and keeping it working unmodified was lower-risk than trying to repurpose code built for a different audience under a different auth model. The new public assistant lives entirely in new files (`PublicAssistantService`, `PublicAssistantController`, `SiteIngestionService`, `AssistantRateLimiter`, a new `chat-service`-local `CmsAdminGuard`, `VectorStoreConfig`, `Sites`) and a new `publicAssistantChatClient` bean alongside (not replacing) the old `chatClient` bean.
- **One Qdrant collection per site, for real**: `spring.ai.vectorstore.qdrant`'s single-collection auto-configuration (`QdrantVectorStoreAutoConfiguration`) is excluded, replaced by `VectorStoreConfig` building a `Map<String, VectorStore>` — one `QdrantVectorStore` per site code (`cms_en`, `cms_vi`, `cms_ja`, `cms_ko`, `cms_de`), sharing one hand-built `QdrantClient` bean. This is structurally the same collection-per-site guarantee Phase 3 and Phase 1 gave media/auth — not a shared collection filtered by a `site` field, which the decision log explicitly rejected.
- **Found and fixed a self-inflicted regression before it shipped**: excluding the single-collection autoconfiguration also removed the one `VectorStore` bean the untouched `AssistantService`/`IngestionService` still depend on by type — caught by `chat-service`'s own pre-existing `ChatServiceApplicationTests.contextLoads()` test (`NoSuchBeanDefinitionException` for `VectorStore`), not by inspection. Fixed by adding back a single legacy `VectorStore` bean pointed at the original `crm_vi` collection, so the old CRM assistant's dependency is satisfied exactly as before.
- **Ingestion re-sourced from `content-service`, not the classpath `documents/` folder**: `SiteIngestionService` calls `content-service`'s public `GET /v1/home` per site, explicitly setting the `Host` header to that site's real subdomain (e.g. `vn.cmcglobal.com`) — since there's no gateway to preserve a real subdomain on a direct backend-to-backend call, and adding a second, admin-only content-service endpoint just for this would duplicate a contract that already does exactly the right thing. Each list item (stats, services, offices, case studies, insights, awards/certifications/partners, testimonials) becomes one short natural-language `Document`; navigation/footer items are skipped as not useful RAG content. Re-ingesting a site does a full wipe-and-reload of just that site's collection, mirroring the old `IngestionService`'s per-application semantics.
- **Public persona, per site**: `PublicAssistantService` builds the system prompt per request (language + "you are CMC Global's marketing assistant, answer only from context" persona) from `Sites.LANGUAGE`, rather than baking one static prompt into the shared `ChatClient` bean the way the old CRM assistant does — the persona genuinely varies per site, so it can't be a bean-level default.
- **Anonymous by design, with a real gate**: `POST /v1/assistant/ask` is `permitAll()`'d in core-v1's shared `SecurityConfig`. Caught during implementation, not after: core-v1's `SecurityConfig` never disables Spring Security's default anonymous authentication, so an unauthenticated request on a `permitAll` endpoint still arrives with a non-null `AnonymousAuthenticationToken`, not a null `Authentication` — a naive `authentication != null` exemption check would have silently exempted every anonymous visitor from rate limiting. Fixed by checking `authentication.isAuthenticated() && !(authentication instanceof AnonymousAuthenticationToken)`.
- **IP-based rate limiting** (`AssistantRateLimiter`): Redis-backed sliding-ish fixed window (10 requests/minute per server-observed remote address, never a client-supplied header — same rule as Phase 1's login lockout fix), applied only when the check above says the caller is anonymous.
- **Ingest is site-scoped admin, not global-admin-only**: unlike the old CRM `/assist/ingest` (blanket `Roles.ADMIN` check), `POST /v1/assistant/ingest?site=` uses the same `CmsAdminGuard.requireSiteAccess` pattern as content-service/media-service — a `vi`-scoped editor can re-ingest the `vi` site after publishing, without needing a global role.
- **Flagged, not fixed**: `CodeReviewService` still shares the old `chatClient` bean and its Ollama-specific `OllamaChatOptions.builder().disableThinking()` call — untouched, since it's an unrelated internal dev-tool feature, not part of the CMS chatbot. This was already true before this phase and stays true after it.
- Verified: `:auth-service:compileJava`, `:media-service:compileJava`, `:content-service:compileJava`, `:chat-service:compileJava` all succeed together (confirms the new Anthropic starter, the excluded autoconfiguration, and the new beans don't break anything else). `:chat-service:test`'s one existing test (`contextLoads`) still fails in this sandbox — same as Phase 0's documented baseline (no live Redis/Postgres), just now also blocked on a live Qdrant connection for the eager `initializeSchema` check on the 6 vector store beans (was 1 before). No live infra exists in this environment to get past that in any phase so far; this isn't a new limitation, just a wider surface hitting the same one.
- **Not done**: no real Anthropic API key was exercised (no live call ever made — `ANTHROPIC_API_KEY` is an empty-default env var), no live end-to-end ingestion-then-ask test (needs live `content-service` + Qdrant + a real API key), and the "drop the Ollama-specific `disableThinking()` call" bullet only applies to the NEW public assistant path (which never had one to begin with, since it's Claude-backed) — the OLD CRM assistant's call is deliberately left alone per the "flagged, not fixed" note above.
...
### Phase 6 status: partial — 2026-09-18

- **CORS, adapted for "no gateway"**: the plan's own text says "apply the allowlist on the gateway," but Phase 0 already established there is no gateway in this platform. Implemented directly on each resource service instead: a new `CorsConfigurationSource` bean in core-v1's shared `SecurityConfig` (used by `content-service`/`media-service`/`chat-service`), driven by a `cors.allowed-origins` env-configurable property, defaulting to `http://localhost:5173` (admin-app) + `http://localhost:3000` (the Next.js sites' dev port) — real values for the 5 production subdomains get added via `CORS_ALLOWED_ORIGINS` when actually deploying, matching the decision log's "user maintains this list manually."
- **Found and fixed a real pre-existing gap while wiring this up**: `auth-service`'s own `corsConfigurationSource()` bean (for `/oauth2/**`, `/connect/**` — hit directly by the browser during the PKCE flow) used `setAllowedOriginPatterns(List.of("*"))` with `allowCredentials(true)` — a wildcard-with-credentials origin allowance, explicitly against CLAUDE.md's own "CORS allowlist... never wildcard it" rule. This predates this platform (it's Workflow's original config, with a comment saying its gateway used to handle CORS for everything else) and was never touched or flagged in Phases 0–5. Fixed to use the same `cors.allowed-origins` allowlist as everything else. (Confirmed `auth-service` doesn't component-scan `core-v1` at all — plain `@SpringBootApplication` with no widened `scanBasePackages`, unlike the other three services — so this fix had to be made in `auth-service`'s own local `SecurityConfig`, not by relying on the shared core-v1 bean.)
- `docs/api-contract.md` rewritten from a placeholder into the actual current endpoint list across all four services (auth OAuth2 endpoints, content-service's public + 11-resource admin CRUD contract, media-service's upload/library/video endpoints, chat-service's new public assistant + untouched legacy `/assist/**`), replacing the "not yet generated" stub.
- **Not done, deferred — same as every prior phase's own admission**: the "full security pass... done properly, not deferred further" bullet did **not** happen as a systematic audit; the user is still doing that separately, per the decision log's original deferral. What DID happen, cumulatively, across Phases 1–6: the login-lockout IP-bypass fix and the `"vn"`/`"vi"` site-code seeding bug (Phase 1), the chunked-upload cross-site file leak (Phase 3), and the CORS wildcard (this phase) — all found by reading code in the course of doing the actual phase work, not from a dedicated audit pass. That distinction matters: a real audit might still find more.
- **Not done**: load testing (needs a live deployed stack, which no phase in this session has had), and the end-to-end test ("publish in admin UI → confirm via public API → confirm Next.js picks it up") — same `docker-compose up` limitation noted in every phase since Phase 0. Everything in this platform has been verified at the compile/unit-test/build level only; nothing has been run live end-to-end.
